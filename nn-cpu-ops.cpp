...
static inline vfloat32m1_t expf_rvv(vfloat32m1_t x) {
    //size_t vl = 8;  // K1: float32, VLEN = 256 bits → 8 x float32
    size_t vl = __riscv_vsetvl_e32m1(8); 

    const vfloat32m1_t ln2 = __riscv_vfmv_v_f_f32m1(0.69314718056f, vl);
    const vfloat32m1_t inv_ln2 = __riscv_vfmv_v_f_f32m1(1.44269504089f, vl);
    const vfloat32m1_t c1 = __riscv_vfmv_v_f_f32m1(1.0f, vl);
    const vfloat32m1_t c2 = __riscv_vfmv_v_f_f32m1(0.5f, vl);
    const vfloat32m1_t c3 = __riscv_vfmv_v_f_f32m1(0.1666666667f, vl);
    const vfloat32m1_t c4 = __riscv_vfmv_v_f_f32m1(0.04166666667f, vl);
    const vfloat32m1_t c5 = __riscv_vfmv_v_f_f32m1(0.008333333333f, vl);

    // clamp
    x = __riscv_vfmin_vf_f32m1(x, 88.0f, vl);
    x = __riscv_vfmax_vf_f32m1(x, -88.0f, vl);

    vfloat32m1_t kf = __riscv_vfmacc_vv_f32m1(__riscv_vfmv_v_f_f32m1(0.5f, vl), x, inv_ln2, vl);
    vint32m1_t k = __riscv_vfcvt_x_f_v_i32m1(kf, vl);
    kf = __riscv_vfcvt_f_x_v_f32m1(k, vl);

    vfloat32m1_t r = __riscv_vfnmsac_vv_f32m1(x, ln2, kf, vl);

    vfloat32m1_t p = c5;
    p = __riscv_vfmadd_vv_f32m1(p, r, c4, vl);
    p = __riscv_vfmadd_vv_f32m1(p, r, c3, vl);
    p = __riscv_vfmadd_vv_f32m1(p, r, c2, vl);
    p = __riscv_vfmadd_vv_f32m1(p, r, c1, vl);

    vint32m1_t k_exp = __riscv_vadd_vx_i32m1(k, 127, vl);
    vint32m1_t exp_bits = __riscv_vsll_vx_i32m1(k_exp, 23, vl);
    vfloat32m1_t two_k = __riscv_vreinterpret_v_i32m1_f32m1(exp_bits);

    return __riscv_vfmul_vv_f32m1(p, two_k, vl);
}
#endif*/
...
static void matmul_Q80_Q40_F32(
    float *output, 
    const NnBlockQ80 *x, 
    const NnBlockQ40 *w, 
    const NnUint n, 
    ...
    const NnUint threadIndex)
{
    ...
    for (NnUint i = start; i < end; i++) {
        ...
        for (unsigned j = 0; j < nBlocks; j++) {
            const NnBlockQ40 *wb = &w[i * nBlocks + j];
            const NnBlockQ80 *xb = &x[j];
            float scaling_fector = CONVERT_F16_TO_F32(wb->d) * CONVERT_F16_TO_F32(xb->d);

            vint8m1_t x256 = __riscv_vle8_v_i8m1(xb->qs, n32);
            vuint8m1_t u_weight = __riscv_vle8_v_u8m1(wb->qs, n16);
            vuint8m1_t u_weight_low = __riscv_vand_vx_u8m1(u_weight, 0x0F, n16);
            vuint8m1_t u_weight_high = __riscv_vsrl_vx_u8m1(u_weight, 4, n16);

            vint8m1_t weight_low = __riscv_vsub_vx_i8m1(__riscv_vreinterpret_v_u8m1_i8m1(u_weight_low), 8, n16);
            vint8m1_t weight_high = __riscv_vsub_vx_i8m1(__riscv_vreinterpret_v_u8m1_i8m1(u_weight_high), 8, n16);
            vint8m1_t w256 = __riscv_vslideup_vx_i8m1(weight_low, weight_high, 16, n32);

            vint16m2_t product = __riscv_vwmul_vv_i16m2(x256, w256, n32);

            vint32m1_t acc = __riscv_vmv_v_x_i32m1(0, n32);  
            acc = __riscv_vwredsum_vs_i16m2_i32m1(product, acc, n32);

            int32_t result;
            __riscv_vse32_v_i32m1(&result, acc, 1);
            sum += (float)result * scaling_fector;
        }
...
/*#elif defined(__riscv_vector)
    const size_t vl = 8;  // 固定處理 8 個 float32
    const unsigned int count = end - start;
    const unsigned int rvvEnd = end - (count % vl);  // 每 8 個 f32

    const vfloat32m1_t zero = __riscv_vfmv_v_f_f32m1(0.0f, vl);
    const vfloat32m1_t one  = __riscv_vfmv_v_f_f32m1(1.0f, vl);
    for (; i < rvvEnd; i += vl) {
        vfloat32m1_t x = __riscv_vle32_v_f32m1(&output[i], vl);
        vfloat32m1_t neg_x = __riscv_vfsub_vv_f32m1(zero, x, vl);
        vfloat32m1_t exp_negx = expf_rvv(neg_x);
        vfloat32m1_t denom = __riscv_vfadd_vv_f32m1(exp_negx, one, vl);
        vfloat32m1_t recip = __riscv_vfdiv_vv_f32m1(one, denom, vl);
        vfloat32m1_t result = __riscv_vfmul_vv_f32m1(x, recip, vl);
...
/*#elif defined(__riscv_vector)
    const unsigned int count = end - start;
    const unsigned int rvvEnd = end - (count % 8);
    size_t vl = __riscv_vsetvl_e32m1(8);
    for(; i < rvvEnd; i += 8){
